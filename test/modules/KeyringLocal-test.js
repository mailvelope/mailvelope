...
describe('KeyringLocal unit tests', () => {
  ...
  describe('getKeys', () => {
    it('should get all public and private keys', async() => {
      ...
      expect(keys.length).to.be.at.least(4);
  ...
  describe('generateKey', () => {
    it('should Generate a new PGP keypair and optionally upload to the publickey server', function() {
      this.timeout(5000);
      return keyRing.generateKey({keyAlgo: 'RSA', numBits: 2048, userIds: [{email: 'g.freeman@blackmesa.org', fullName: 'Gordon Freeman'}, {email: 'freeman@mailvelope.com', fullName: 'G. Freeman'}], passphrase: 'blackmesa', uploadPublicKey: false, keyExpirationTime: 31536000}).then(async() => {
