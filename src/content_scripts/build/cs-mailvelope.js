/*!
 * jQuery JavaScript Library v1.8.0 -deprecated,-ajax,-ajax/jsonp,-ajax/script,-ajax/xhr
 * http://jquery.com/
 *
 * Includes Sizzle.js
 * http://sizzlejs.com/
 ...
 *
 * Date: Wed Aug 15 2012 15:43:22 GMT+0200 (CEST)
 */
  ...
  encryptFrame.prototype = {
    ...
    _init: function(element) {
      ...
      if (!this._editElement.is('body') && this._editElement.closest('body').data(constant.DYN_IFRAME)) {
        ...
        if (!html.data('G-STYLE')) {
          var style = $('<link/>', {
            rel: 'stylesheet',
            href: chrome.extension.getURL('framestyles.css')
          });
          // add style
          html.find('head').append(style);
          // set marker
          html.data('G-STYLE', true);
        }
      }
    },
    ...
    _renderFrame: function(expanded) {
      ...
      this._eFrame = $('<div/>', {
        id: 'eFrame' + that.id,
        'class': 'g-encrypt-frame',
        html: '<a class="g-frame-close">×</a><button class="g-encrypt-button"></button>'
      });
      ...
      this._refreshPosIntervalID = window.setInterval(this._setFrameDim.bind(this), 1000);
      this._eFrame.find('.g-encrypt-button').on('click', this._onEncryptButton.bind(this));
      this._eFrame.find('.g-frame-close').on('click', this._closeFrame.bind(this));
      if (!expanded) {
    ...
    _onEncryptButton: function() {
      if (this._eFrame.find('.g-encrypt-button').hasClass('g-encrypt-undo')) {
        this._resetEmailText();
        this._eFrame.find('.g-encrypt-button').removeClass('g-encrypt-undo');
      } else {
        this.showEncryptDialog();
      }
      return false;
    ...
    _expandFrame: function(callback) {
      this._eFrame.hide();
      this._eFrame.find('.g-encrypt-button').hide();
      this._eFrame.addClass('g-encrypt-frame-expanded');
    ...
    _showToolbar: function() {
      this._eFrame.fadeOut((function() {
        this._eFrame.find('.g-encrypt-button').show();
        this._eFrame.removeClass('g-encrypt-frame-expanded');
    ...
    _showDialog: function() {
      ...
      this._eDialog = $('<iframe/>', {
        id: 'eDialog' + that.id,
        'class': 'g-frame-dialog',
        src: chrome.extension.getURL('views/encryptDialog.html?id=' + that.id),
        frameBorder: 0, 
        scrolling: 'no'
      });
    ...
    _establishConnection: function() {
      var that = this;
      this._port = chrome.extension.connect({name: 'eFrame-' + that.id});
    },
      ...
      this._port.onMessage.addListener(function(msg) {
        ...
        switch (msg.event) {
          ...
          case 'encrypted-message':
            ...
            that._removeDialog();
            that._eFrame.find('.g-encrypt-button').addClass('g-encrypt-undo');
            break;
          default:
            console.log('unknown event');
        }
...
(document.mailvelopeControl || function() {
  ...
  function getTabid() {
    chrome.extension.sendMessage({event: "get-tabid"}, function(response) {
  ...
  function findPGPTag(regex) {
    ...
    nodeList = $(nodeList).filter(function() {
      var element = $(this);
      // visibility check does not works on text nodes
  ...
  document.mailvelopeControl = true;
  init();
