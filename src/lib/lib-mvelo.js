...
mvelo.util.getHostname = async function(url) {
  const hostname = await this.sendOffscreenMessage('get-hostname', url);
  return hostname;
};

mvelo.util.getProtocol = async function(url) {
  const protocol = await this.sendOffscreenMessage('get-protocol', url);
  return protocol;
};

mvelo.util.getDomain = async function(url) {
  const domain = await this.sendOffscreenMessage('get-domain', url);
  return domain;
};
