...
(function(app) {
  ...
  function initPrimaryKeyTab($keyData) {
    ...
    if (expirationDate === 'false') {
      expirationDate = app.l10n.keygrid_key_not_expire;
    } else {
  ...
  function initSubKeysTab(details) {
    ...
    details.subkeys.forEach(function(subkey, index) {
      ...
      if (expDate === false) {
        expDate = app.l10n.keygrid_key_not_expire;
      } else {
  ...
  function setPrimaryKey(primaryKeyId) {
    app.setKeyringAttr(app.keyringId, {
  ...
  function openExportAllDialog() {
    ...
    app.keyring('getArmoredKeys', [[], {pub: true, priv: true, all: true}])
      .then(function(result) {
        ...
        initExport(allKeys, 'all_keys', hasPrivate ? '<b>' + app.l10n.header_warning + '</b> ' + app.l10n.key_export_warning_private : null);
      });
  }

  function deleteKeyEntry() {
    var $entryForRemove;
    var confirmResult = confirm(app.l10n.keygrid_delete_confirmation);
    if (confirmResult) {
  ...
  app.createKeyring = function(keyRingId) {
    mvelo.extension.sendMessage({
  ...
  app.setKeyringAttr = function(keyRingId, keyRingAttr) {
    mvelo.extension.sendMessage({
  ...
  app.event.on('keygrid-reload', reload);
