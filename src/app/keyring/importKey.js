...
(function(exports, app) {
  ...
  function init() {
    ...
    hkpUrlLoad();
    $('#keySearchInput').attr('placeholder', app.l10n.key_import_hkp_search_ph);
  ...
  function onImportKey(keyText, callback) {
    ...
    if (keyText.length > MAX_KEY_IMPORT_SIZE) {
      $('#importAlert').showAlert(app.l10n.key_import_error, app.l10n.key_import_too_big, 'danger', true);
      return;
    }
    ...
    if (keys.length === 0) {
      $('#importAlert').showAlert(app.l10n.key_import_error, app.l10n.key_import_invalid_text, 'danger', true);
      return;
    }
    ...
    .then(function(result) {
      ...
      result.forEach(function(imported) {
        ...
        switch (imported.type) {
          case 'success':
            heading = app.l10n.alert_header_success;
            success = true;
            break;
          case 'warning':
            heading = app.l10n.alert_header_warning;
            break;
          case 'error':
            heading = app.l10n.key_import_error;
            type = 'danger';
            break;
        }
    ...
    .catch(function(error) {
      $('#importAlert').showAlert(app.l10n.key_import_error, error.type === 'error' ? error.message : app.l10n.key_import_exception, 'danger', true);
      if (callback) {
        callback([{type: 'error'}]);
      }
    });
  ...
  app.event.on('hkp-url-update', hkpUrlLoad);
