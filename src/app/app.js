...
(function(exports, $) {
  ...
  function initUI() {
    ...
    .then(function() {
      return app.keyring('getPrivateKeys');
    })
    .then(function(privateKeys) {
      return showSetupView(privateKeys);
    })
    .then(function() {
      return app.getAllKeyringAttr();
    })
  ...
  function switchOptionsUI(keyRingAttr) {
    ...
    switch (window.location.hash) {
      case '#securityLog':
        ...
        app.startSecurityLogMonitoring();
        break;
      case '#general':
      case '#security':
      case '#watchList':
  ...
  function handleRequests(request, sender, sendResponse) {
    switch (request.event) {
      case 'add-watchlist-item':
        ...
        app.addToWatchList(request.site);
        break;
      case 'reload-options':
      ...
      case 'import-key':
        ...
        app.importKey(request.armored, function(result) {
          sendResponse({
            result: result,
            id: request.id
          });
  ...
  app.event.on('keygrid-reload', function() {
    app.keyring('getPrivateKeys')
